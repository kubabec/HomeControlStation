...
script += "case 60:{let value=Math.round((Number(document.getElementById('nozzle1Pin-'+id).value)||0)*1.0);bytes[0]=(value>>0)&255;}{let value=Math.round((Number(document.getElementById('nozzle2Pin-'+id).value)||0)*1.0);bytes[1]=(value>>0)&255;}{let value=Math.round((Number(document.getElementById('outputsActiveHigh-'+id).value)||0)*1.0);bytes[2]=(value>>0)&255;}{let value=(document.getElementById('sensorEnabled-'+id).checked?1:0);bytes[7]=(value>>0)&255;}{let value=Math.round((Number(document.getElementById('wateringDurationSeconds-'+id).value)||0)*1.0);bytes[3]=(value>>0)&255;bytes[4]=(value>>8)&255;}{let value=Math.round((Number(document.getElementById('dryThreshold-'+id).value)||0)*1.0);bytes[5]=(value>>0)&255;}{let value=Math.round((Number(document.getElementById('wetThreshold-'+id).value)||0)*1.0);bytes[6]=(value>>0)&255;}break;";
